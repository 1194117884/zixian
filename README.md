...
后台保存 AI 渠道 Key 前，还必须设置 Secret `ADMIN_CONFIG_KEY`（建议用至少 32 个随机字符）。该值用于 AES-GCM 加密 D1 中的 Key；Key 不会回传到管理页面。每个渠道可以在后台添加最多 12 个账号；生成按任务 ID 均衡分配，遇到 401/403、429、超时或 5xx 上游错误时自动串行切换下一个账号。未设置后台渠道配置时，生成会继续使用现有的模型环境变量。
