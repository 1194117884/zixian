...
后台保存 AI 渠道 Key 前，还必须设置 Secret `ADMIN_CONFIG_KEY`（建议用至少 32 个随机字符）。该值用于 AES-GCM 加密 D1 中的 Key；Key 不会回传到管理页面。后台的每条模型渠道均由管理员填写平台、接口地址、模型名称、Key 和所属等级；同一等级最多可有 36 条渠道，生成按任务 ID 均衡分配，遇到 401/403、429、超时或 5xx 上游错误时自动串行切换下一个账号。平台名包含 `Anthropic` 时按 Anthropic 协议调用，其余按 OpenAI 兼容协议调用。未设置后台渠道配置时，生成会继续使用现有的模型环境变量。
